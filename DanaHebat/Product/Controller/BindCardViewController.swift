...
class BindCardViewController: BaseViewController {
...
extension BindCardViewController {
    ...
    private func getPerInfo() async {
        do {
            ...
            let model = try await viewModel.getCardApi(parameters: parameters)
            if model.illness == 0 {
    ...
    private func savePerInfo(with parameters: [String: String]) async {
        do {
            let model = try await viewModel.saveCardApi(parameters: parameters)
            if model.illness == 0 {
                self.backProductVc()
            }else {
...
extension BindCardViewController: UITableViewDelegate, UITableViewDataSource {
